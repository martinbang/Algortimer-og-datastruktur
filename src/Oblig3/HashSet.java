 ...
 */
public class HashSet<AnyType> extends AbstractCollection<AnyType> implements
		Set<AnyType> {
	/**
	...
	/**
	 * This method is not part of standard Java. Like contains, it checks if x
	 * is in the set. If it is, it returns the reference to the matching object;
	...
	public AnyType getMatch(AnyType x) {
		...
		{
			for (HashEntry e : array[currentPos]) {
				if (e.element.equals(x)) {
					return (AnyType)e.element;
				}
			}
		}
		return null;
	 ...
	 */
	private static boolean isActive(LinkedList<HashEntry>[] arr, int pos) {
		return arr[pos] != null;
	}
	...
	public boolean add(AnyType x) {
		...
		if (array[currentPos] == null)
			occupied++;
		array[currentPos].add(new HashEntry(x, true));
		currentSize++;
		modCount++;
	 ...
	 */
	private void rehash() {
		LinkedList<HashEntry>[] oldArray = array;
		...
		for (int i = 0; i < oldArray.length; i++)
			if (isActive(oldArray, i)) {
				for (HashEntry e : oldArray[i]) {
					add((AnyType)e.element);
				}
			}
	}
	...
	public boolean remove(Object x) {
		AnyType e = getMatch((AnyType)x);
		if (e == null)
			return false;

		getEntry(x).isActive = false;
		currentSize--;
		modCount++;
	...
	private int findPos(Object x) {
		int offset = 1;
		int currentPos = (x == null) ? 0 : Math
				.abs(x.hashCode() % array.length);
	...
	private void allocateArray(int arraySize) {
		array = new LinkedList[nextPrime(arraySize)];
		for (int i = 0; i < array.length; i++) {
	...
	private HashEntry getEntry(Object x) {
		int currentPos = findPos(x);
	...
	private LinkedList<HashEntry>[] array;
}
